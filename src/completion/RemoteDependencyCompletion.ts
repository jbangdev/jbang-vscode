  ...
  async provideCompletionItems(
    ...
    if (!json) {
      ...
      try {
        ...
        console.error(
          `Dependencies search failed: ${e.message ? e.message : e}`
        );
      }
    ...
    switch (parts.length) {
      ...
      case 4: //has groupid, artifactId, version and classifier
        ...
        result = toCompletionList(
          json,
          new Range(versionStart, end),
          saveCommand,
          toVersionCompletionItem
        );
        break;
      default:
...
function toVersionCompletionItem(
  gav: any,
  index: number,
  range: Range,
  command?: Command
