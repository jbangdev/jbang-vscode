...
export class LocalDependencyCompletion implements CompletionParticipant {
  applies(lineText: string, position: Position): boolean {
    return (
      DEPS.matches(lineText, true) && position.character > DEPS.prefix().length
    );
  }
  ...
  async provideCompletionItems(
    ...
    let start = TextHelper.findStartPosition(
      lineText,
      position,
      prefix,
      delimiterFunc
    );
    const currText = lineText
      .substring(start.character, position.character)
      .trim();
    ...
    if (isAbsolute) {
      ...
      targetDir = path.join(
        targetDir,
        currText.substring(0, lastFolderDelimIdx)
      ); // a/b
    }
    ...
    let completionItems = fileEntries
      .filter((entry) => {
        return (
          entry.name.startsWith(lastSegment) &&
          !(targetDir === currDir && entry.name === fileName)
        );
      })
      .filter((entry) => {
        return (
