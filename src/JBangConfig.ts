import { commands, ConfigurationTarget, ExtensionContext, window, workspace, WorkspaceConfiguration } from "vscode";
...
export class JBangConfig {
    ...
    initialize(context: ExtensionContext) {
		context.subscriptions.push (
            workspace.onDidChangeConfiguration(params => {
                ...
                if (hasConfigKeyChanged('import.jbang.projectPerScript', this.oldJavaConfig, newJavaConfig)) {
    ...
    public getJBangHome(): string|undefined {
        return (this.getJBangConfiguration().get(JBangSettings.HOME) as string|undefined)?.trim();
    }
...
function hasConfigKeyChanged(key:string, oldConfig: WorkspaceConfiguration, newConfig:WorkspaceConfiguration): boolean {
