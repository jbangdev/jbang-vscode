...
export default class JBangAddMissingDependencyWizard {
  ...
  private async selectDependency(
    ...
    state: Partial<SelectDependencyState>
  ) {
...
const compareArtifactVersions = (
  a: { g: string; a: string; v: string },
  b: { g: string; a: string; v: string }
) => {
...
function compareNamespaces(reference: string, a: string, b: string): number {
  ...
  const commonPartsA = aParts.filter(
    (part, index) => part === referenceParts[index]
  );
  const commonPartsB = bParts.filter(
    (part, index) => part === referenceParts[index]
  );
