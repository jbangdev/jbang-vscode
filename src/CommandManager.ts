...
export class CommandManager {
  public async initialize(context: ExtensionContext) {
    context.subscriptions.push(
      commands.registerCommand(
        "jbang.synchronize",
        this.synchronizeJBangRequest
      ),
      commands.registerCommand(
        "jbang.script.run",
        handleCommand.bind(this, JBangRunner.runJBang)
      ),
      commands.registerCommand(
        "jbang.script.export.native",
        handleCommand.bind(this, JBangRunner.exportNative)
      ),
      commands.registerCommand(
        "jbang.script.app.install",
        handleCommand.bind(this, JBangInstallAppWizard.open)
      ),
      ...
      commands.registerCommand(
        JBANG_ADD_MISSING_DEPENDENCY,
        this.handleMissingDependencyCommand.bind(this)
      )
    );
  }
  ...
  private async synchronizeJBangRequest(uris?: Uri | Uri[]) {
    ...
    if (!uris) {
      ...
      if (activeFileUri) {
        //&& isJBangFile(activeFileUri.fsPath)
    ...
    return commands.executeCommand(
      JAVA_EXECUTE_WORKPACE_COMMAND,
      JDTLS_JBANG_SYNCHRONIZE_COMMAND,
      resources
    );
  }
